...
export default function NewSessionContent() {
  ...
  const {
    ...
    defaultValues: { studentId: preselectedId, score: '', topic: '', observation: '' },
  });
  ...
  useEffect(() => {
    ...
    if (studentSessions.length > 0 && !isNaN(score)) {
      const close = studentSessions.find(
        (s) => Math.abs(s.score - score) <= 8
      );
      setIsCacheHit(!!close);
    } else {
      setIsCacheHit(false);
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6">
            ...
            <div className="mt-4 p-3 rounded-xl bg-secondary/60 border border-border flex items-center justify-between gap-4 flex-wrap animate-fade-in">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{selectedStudent.grade} · {selectedStudent.sessionCount} sessions completed</p>
                </div>
              </div>
              <div className="flex items-center gap-4 text-right">
                ...
                <div>
                  <p className="text-sm font-600 text-foreground">{selectedStudent.lastSessionDate !== '—' ? selectedStudent.lastSessionDate : 'No sessions yet'}</p>
        ...
        <div className="card-elevated p-5">
          ...
          <div className="flex flex-col gap-4">
            <div>
              <label className="block text-sm font-600 text-foreground mb-1.5">
                Topic / Pillar <span className="text-negative">*</span>
              </label>
              <p className="text-xs text-muted-foreground mb-2">
                Select the subject area or developmental pillar for this session
              </p>
              <select
        ...
        <div className="card-elevated p-5">
          ...
          <div>
            <label className="block text-sm font-600 text-foreground mb-1.5">
              Comments & Notes <span className="text-negative">*</span>
            </label>
            <p className="text-xs text-muted-foreground mb-2">
              Describe what you observed during this session. Be specific — the AI generates better
              guidance with richer context. Include behavior, attitude, specific struggles, and any
              notable moments.
            </p>
            <textarea
              className="input-mystic resize-none leading-relaxed"
              rows={6}
              placeholder="e.g. The student demonstrated strong recall of vocabulary but struggled to construct coherent arguments in the essay section. They showed visible frustration when asked to revise their thesis..."
              {...register('observation', {
                required: 'Observations are required',
                minLength: { value: 30, message: 'Please provide at least 30 characters of observation' },
              })}
            />
            <div className="flex items-center justify-between mt-1.5">
              {errors.observation ? (
                ...
                <p className="text-xs text-muted-foreground">
                  More detail → richer AI analysis
                </p>
              )}
              <p className={`text-xs tabular-nums ${watchedObservation.length > 150 ? 'text-info' : 'text-muted-foreground'}`}>
                {watchedObservation.length} chars
                {watchedObservation.length > 150 && ' · Enhanced model'}
              </p>
            </div>
          </div>
        </div>
        ...
        {watchedTopic && (
          <div className="animate-fade-in">
            <AIRoutingIndicator
              observationLength={watchedObservation.length}
              topic={watchedTopic}
          ...
          <div className="card-elevated-md p-8 flex flex-col items-center gap-4 max-w-sm text-center">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1 leading-relaxed">
                Our veteran educator AI is reviewing the session data and crafting personalized guidance...
              </p>
            </div>
