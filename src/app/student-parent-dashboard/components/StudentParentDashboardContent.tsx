...
function getQuestTier(completedCount: number) {
  ...
  if (completedCount >= 5) {
    return {
      ...
      nextAt: 15,
      progress: Math.round(((completedCount - 5) / 10) * 100),
    };
  }
  return {
    name: 'Explorer',
        ...
        {[1, 2, 3, 4, 5].map((star) => (
          ...
          >
            <Icon
              name="StarIcon"
              size={22}
              variant={star <= value ? 'solid' : 'outline'}
              className={star <= value ? 'text-amber-400' : 'text-muted-foreground'}
            />
          </button>
        ))}
      </div>
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-7 gap-1">
        ...
        {cells.map((day, idx) => {
          ...
          return (
            <div
              key={dateStr}
              className={`aspect-square flex items-center justify-center rounded-lg text-xs font-600 transition-colors ${
                status === 'present' ?'bg-[#C8F0D8] text-green-800 border border-green-200'
                  : status === 'absent' ?'bg-[#F5C6C9] text-red-800 border border-red-200' :'text-foreground hover:bg-secondary'
              }`}
            >
              {day}
            </div>
      ...
      <div className="flex items-center gap-4 text-xs text-muted-foreground">
        <div className="flex items-center gap-1.5">
          <div className="w-3 h-3 rounded bg-[#C8F0D8] border border-green-200" />
          Present ({presentCount})
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-3 h-3 rounded bg-[#F5C6C9] border border-red-200" />
          Absent ({absentCount})
        </div>
      </div>
    </div>
...
export default function StudentParentDashboardContent() {
  ...
  const [activeTab, setActiveTab] = useState<
    'overview' | 'calendar' | 'report' | 'feedback' | 'parent'
  >('overview');
  ...
  const [feedbackForm, setFeedbackForm] = useState({
    mentor_interaction_score: 5,
    active_listening_score: 5,
    teaching_clarity_score: 5,
    fruitful_comments: '',
  });
  ...
  const [leaderboard, setLeaderboard] = useState<
    { name: string; count: number; userId: string }[]
  >([]);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // If mentor role, redirect to mentor dashboard
      ...
      if (profile.student_id) {
        ...
        if (student) {
          ...
          // Load parent observations for leaderboard
          ...
          if (allObs) {
            ...
            allObs.forEach((o) => {
              countMap.set(o.submitted_by, (countMap.get(o.submitted_by) || 0) + 1);
            });
      ...
      // Load parent observations by this user
      const { data: parentObs } = await supabase
        .from('parent_observations')
        .select('*')
      ...
      // Load mentor feedback by this user
  ...
  const handleSubmitReflection = async () => {
    if (!reflectionForm.learned_this_week.trim()) {
      toast.error('Please fill in what you learned this week.');
      return;
    }
  ...
  const handleSubmitFeedback = async () => {
    if (!studentProfile) {
      toast.error('No student profile linked.');
      return;
    }
  ...
  const handleSubmitParentObservation = async () => {
    if (!parentForm.observation_text.trim()) {
      toast.error('Please add your observation notes.');
      return;
    }
    if (!studentProfile) {
      toast.error('No student profile linked.');
  ...
  const tabs = [
    { id: 'overview' as const, label: 'My Quest', icon: 'SparklesIcon' },
    { id: 'calendar' as const, label: 'Calendar', icon: 'CalendarDaysIcon' },
    { id: 'report' as const, label: 'Report Card', icon: 'DocumentTextIcon' },
    { id: 'feedback' as const, label: 'Mentor Feedback', icon: 'StarIcon' },
    { id: 'parent' as const, label: 'Parent Hub', icon: 'UserGroupIcon' },
  ];
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 justify-between mb-6">
        ...
        {/* Quest Tier Badge */}
        <div
          className={`flex items-center gap-3 px-4 py-2.5 rounded-2xl border-2 shadow-lg ${tier.bgColor} ${tier.glowClass}`}
        >
          ...
          <div
            className={`w-8 h-8 rounded-full bg-gradient-to-br ${tier.color} flex items-center justify-center text-white text-xs font-700 shadow-md`}
          >
            {completedTasks}
          </div>
        </div>
          ...
          <div className="card-mystic p-5">
            ...
            {tier.nextAt ? (
              <p className="text-xs text-muted-foreground">
                Complete {tier.nextAt - completedTasks} more tasks to reach the next tier!
              </p>
            ) : (
              <p className="text-xs text-muted-foreground">
                🎉 You have reached the highest tier — Trailblazer!
              </p>
            )}
          </div>
                    ...
                    <span
                      className={`text-xs font-600 px-2 py-0.5 rounded-full flex-shrink-0 ${
                        task.status === 'Completed'
                          ...
                          : task.status === 'In Progress' ?'bg-info/10 text-info' :'bg-muted text-muted-foreground'
                      }`}
                    >
                      {task.status}
                    </span>
                ...
                {meetings.slice(0, 3).map((m) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {formatDate(m.meeting_date)} at {m.meeting_time}
                      </p>
                      {m.notes && (
                ...
                {meetings.map((m) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {m.notes && (
                        <p className="text-xs text-muted-foreground mt-1 leading-relaxed">{m.notes}</p>
                      )}
                    </div>
                    {m.jitsi_url && (
                      <a
      ...
      {activeTab === 'report' && (
        <div className="flex flex-col gap-6">
          ...
          {/* Student Reflective Insights Form */}
          <div className="card-mystic p-5">
            <div className="flex items-center gap-2 mb-4">
            ...
            <div className="flex flex-col gap-4">
              <div>
                <label className="block text-sm font-600 text-foreground mb-1.5">
                  What did I learn this week? <span className="text-negative">*</span>
                </label>
                <textarea
                  className="input-mystic min-h-[90px] resize-none"
                  placeholder="Share your key learnings, discoveries, or insights from this week..."
                  value={reflectionForm.learned_this_week}
                  onChange={(e) =>
                    setReflectionForm((f) => ({ ...f, learned_this_week: e.target.value }))
                  }
                />
              </div>
              <div>
                <label className="block text-sm font-600 text-foreground mb-1.5">
                  Which aspects do I need to work on further?
                </label>
                <textarea
                  className="input-mystic min-h-[80px] resize-none"
                  placeholder="Identify areas where you feel you need more practice or understanding..."
                  value={reflectionForm.needs_work}
                  onChange={(e) =>
                    setReflectionForm((f) => ({ ...f, needs_work: e.target.value }))
                  }
                />
              </div>
              <div>
                <label className="block text-sm font-600 text-foreground mb-1.5">
                  How was my team dynamics, and what was the best thing about my team?
                </label>
                <textarea
                  className="input-mystic min-h-[80px] resize-none"
                  placeholder="Reflect on your teamwork experience, collaboration, and what made your team shine..."
                  value={reflectionForm.team_dynamics}
                  onChange={(e) =>
                    setReflectionForm((f) => ({ ...f, team_dynamics: e.target.value }))
                  }
                />
              </div>
              <button
        ...
        <div className="flex flex-col gap-6">
          <div className="card-mystic p-5">
            ...
            <div className="flex flex-col gap-5">
              <StarRating
                label="Mentor Interaction & Responsiveness"
                value={feedbackForm.mentor_interaction_score}
                onChange={(v) => setFeedbackForm((f) => ({ ...f, mentor_interaction_score: v }))}
              />
              <StarRating
                label="Active Listening"
                value={feedbackForm.active_listening_score}
                onChange={(v) => setFeedbackForm((f) => ({ ...f, active_listening_score: v }))}
              />
              <StarRating
                label="Teaching Clarity"
                value={feedbackForm.teaching_clarity_score}
                onChange={(v) => setFeedbackForm((f) => ({ ...f, teaching_clarity_score: v }))}
              />
              <div>
                <label className="block text-sm font-600 text-foreground mb-1.5">
                  Additional Comments
                ...
                <textarea
                  ...
                  placeholder="Share any specific feedback, suggestions, or appreciation for your mentor..."
                  value={feedbackForm.fruitful_comments}
                  onChange={(e) =>
                    setFeedbackForm((f) => ({ ...f, fruitful_comments: e.target.value }))
                  }
                />
              </div>
              <button
              ...
              >
                {submittingFeedback ? (
                  ...
                  <><Icon name="PaperAirplaneIcon" size={15} /> Submit Feedback</>
                )}
              </button>
            </div>
          </div>
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="card-mystic p-5">
            ...
            <div className="flex flex-col gap-4">
              <div>
                <label className="block text-sm font-600 text-foreground mb-1.5">
                  Home Progress Notes <span className="text-negative">*</span>
                </label>
                <textarea
                  className="input-mystic min-h-[100px] resize-none"
                  placeholder="Share observations about your child's progress at home — study habits, attitude, challenges..."
                  value={parentForm.observation_text}
                  onChange={(e) =>
                    setParentForm((f) => ({ ...f, observation_text: e.target.value }))
                  }
                />
              </div>
              <div>
                ...
                <textarea
                  className="input-mystic min-h-[80px] resize-none"
                  placeholder="How has the Luminar's Guide program impacted your child? Any suggestions?"
                  value={parentForm.program_experience}
                  onChange={(e) =>
                    setParentForm((f) => ({ ...f, program_experience: e.target.value }))
                  }
                />
              </div>
              <button
                className="btn-primary self-start"
                onClick={handleSubmitParentObservation}
                disabled={submittingParent}
              >
                {submittingParent ? (
                  <><Icon name="ArrowPathIcon" size={15} className="animate-spin" /> Submitting...</>
                ) : (
                  <><Icon name="PaperAirplaneIcon" size={15} /> Submit Observation</>
                )}
              </button>
            </div>
          </div>
                  ...
                  <div
                    ...
                    className={`flex items-center gap-3 p-3 rounded-xl border ${
                      idx === 0
                        ...
                        : idx === 2
                        ? 'bg-orange-50 border-orange-200' :'bg-secondary/40 border-border'
                    }`}
                  >
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-800 flex-shrink-0 ${
                        idx === 0
                          ...
                          : idx === 2
                          ? 'bg-orange-400 text-white' :'bg-muted text-muted-foreground'
                      }`}
                    >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {entry.count} observation{entry.count !== 1 ? 's' : ''}
                      </p>
                    </div>
